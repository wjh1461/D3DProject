...
public:
	...
	CreateApp(const CreateApp& rhs) = delete;
	CreateApp& operator=(const CreateApp& rhs) = delete;
	~CreateApp();
...
CreateApp::CreateApp(HINSTANCE hInstance) : D3DApp(hInstance)
{
}
...
void CreateApp::Update(const GameTimer& gt)
{
	UpdateCamera(gt);
...
void CreateApp::Draw(const GameTimer& gt)
{
...
void CreateApp::UpdateCamera(const GameTimer& gt)
{
...
void CreateApp::UpdateObjectCBs(const GameTimer& gt)
{
...
void CreateApp::UpdateMaterialCBs(const GameTimer& gt)
{
...
void CreateApp::UpdateMainPassCB(const GameTimer& gt)
{
...
void CreateApp::BuildRootSignature()
{
	/*
...
void CreateApp::BuildDescriptorHeaps()
{
	// SRV 힙 생성.
...
void CreateApp::BuildShadersAndInputLayout()
{
	HRESULT hr = S_OK;

	mShaders["standardVS"] = d3dUtil::CompileShader(L"Shaders\\chapter9\\Default.hlsl", nullptr, "VS", "vs_5_0");
	mShaders["opaquePS"] = d3dUtil::CompileShader(L"Shaders\\chapter9\\Default.hlsl", nullptr, "PS", "ps_5_0");
...
void CreateApp::BuildShapeGeometry()
{
...
void CreateApp::BuildFrameResources()
{
...
void CreateApp::BuildRenderItems()
{
...
void CreateApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
	...
	for (size_t i = 0; i < ritems.size(); i++)
	{
		...
		D3D12_GPU_VIRTUAL_ADDRESS matCBAddress = matCB->GetGPUVirtualAddress() + (UINT64)ri->Mat->MatCBIndex * matCBByteSize;
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 6> CreateApp::GetStaticSamplers()
{
